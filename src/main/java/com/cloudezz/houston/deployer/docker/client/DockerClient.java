...
public class DockerClient {
	...
	private static final int STATUS_NO_ERROR = 204;

	private static final int STATUS_NO_SUCH_ENTITY = 404;
	...
	public Info info() throws CloudezzDeployException {
		return textRestTemplate.getForObject(dockerDeamonUrl + "/info", Info.class);
	}

	public Version version() throws CloudezzDeployException {
	...
	public void pull(String repository) throws CloudezzDeployException {
		this.pull(repository, null, null);
	}

	public void pull(String repository, String tag) throws CloudezzDeployException {
		this.pull(repository, tag, null);
	}

	public void pull(String repository, String tag, String registry) throws CloudezzDeployException {
		...
		restTemplate.exchange(dockerDeamonUrl + "/images/create?tag={tag}&fromImage={fromImage}&registry={registry}",
				HttpMethod.POST, null, String.class, params);
	}

	public List<SearchItem> search(String search) throws CloudezzDeployException {
		SearchItem[] response = restTemplate.getForObject(dockerDeamonUrl + "/images/search?term={search}",
				SearchItem[].class, search);
		return Arrays.asList(response);
	}

	public void removeImage(String imageId) throws CloudezzDeployException {
	...
	public void removeImages(List<String> images) throws CloudezzDeployException {
	...
	public String getVizImages() throws CloudezzDeployException {
		return restTemplate.getForObject(dockerDeamonUrl + "/images/viz", String.class);
	}

	public List<Image> getImages() throws CloudezzDeployException {
		return this.getImages(null, false);
	}

	public List<Image> getImages(boolean allContainers) throws CloudezzDeployException {
		return this.getImages(null, allContainers);
	}

	public List<Image> getImages(String name) throws CloudezzDeployException {
		return this.getImages(name, false);
	}

	public List<Image> getImages(String name, boolean allImages) throws CloudezzDeployException {
		Map<String, String> params = new HashMap<String, String>();
	...
	public ImageInspectResponse inspectImage(String imageId) throws CloudezzDeployException {
		return restTemplate.getForObject(dockerDeamonUrl + "/images/{imageId}/json", ImageInspectResponse.class,
				imageId);
	}
	...
	public List<Container> listContainers(boolean listAll) {
		Container[] response = restTemplate.getForObject(dockerDeamonUrl + "/containers/json?all={all}",
				Container[].class, listAll);
		return Arrays.asList(response);
	}

	public Container getContainer(String containerId) {
		Container response = restTemplate.getForObject(dockerDeamonUrl + " /containers/" + containerId + "/json",
				Container.class);
		return response;
	}

	public Container createAndGetContainer(ContainerConfig containerConfig) throws CloudezzDeployException {
		...
		if (containerCreateResponse.getId() != null) {
			Container container = getContainer(containerCreateResponse.getId());
			return container;
		}
		return null;
	}

	public ContainerCreateResponse createContainer(ContainerConfig containerConfig) throws CloudezzDeployException {
		return createContainer(containerConfig, null);
	}

	public ContainerCreateResponse createContainer(ContainerConfig containerConfig, String containerName)
			throws CloudezzDeployException {
		final HttpHeaders requestHeaders = new HttpHeaders();
		requestHeaders.setContentType(MediaType.APPLICATION_JSON);
		requestHeaders.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
		final HttpEntity<ContainerConfig> requestEntity = new HttpEntity<ContainerConfig>(containerConfig,
				requestHeaders);

		ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
		try {
			new ObjectMapper().writeValue(outputStream, containerConfig);
			LOGGER.debug("Creating a container with the following configuration: {}.",
		...
		String response = restTemplate.postForObject(dockerDeamonUrl + "/containers/create" + containerParameter,
	...
	public void startContainer(String containerId) throws CloudezzDeployException {
		this.startContainer(containerId, null);
	}

	public boolean startContainer(String containerId, HostConfig hostConfig) throws CloudezzDeployException {
		// restTemplate.postForLocation(dockerDeamonUrl +
		// "/containers/{containerId}/start", hostConfig, containerId);

		// to be tested
		ResponseEntity<String> response = restTemplate.postForEntity(dockerDeamonUrl
				+ "/containers/{containerId}/start", hostConfig, String.class, containerId);
		HttpStatus status = response.getStatusCode();
		if (status.value() == STATUS_NO_ERROR) {
			return true;
		} else {
			return false;
		}
	...
	public ContainerInspectResponse inspectContainer(String containerId) throws CloudezzDeployException {
		return restTemplate.getForObject(dockerDeamonUrl + "/containers/{containerId}/json",
				ContainerInspectResponse.class, containerId);
	}

	public void removeContainer(String container) throws CloudezzDeployException {
		this.removeContainer(container, false);
	}

	public void removeContainer(String containerId, boolean removeVolumes) throws CloudezzDeployException {
		Preconditions.checkState(!StringUtils.isEmpty(containerId), "Container ID can't be empty");

		try {
			restTemplate.delete(dockerDeamonUrl + "/containers/{containerId}?v={removeVolumes}", containerId,
					removeVolumes ? "1" : "0");
		} catch (HttpClientErrorException e) {
	...
	public void removeContainers(List<String> containers, boolean removeVolumes) throws CloudezzDeployException {
	...
	public ContainerWaitResponse waitContainer(String containerId) throws CloudezzDeployException {
	...
	public InputStream logContainer(String containerId) throws CloudezzDeployException {
		return logContainer(containerId, false);
	}

	public InputStream logContainerStream(String containerId) throws CloudezzDeployException {
		return logContainer(containerId, true);
	}

	private InputStream logContainer(String containerId, boolean stream) throws CloudezzDeployException {
	...
	public List<ChangeLog> containterDiff(String containerId) throws CloudezzDeployException {
		ChangeLog[] response = restTemplate.getForObject(dockerDeamonUrl + "/containers/{containerId}/changes",
				ChangeLog[].class, containerId);
		return Arrays.asList(response);
	}

	public void stopContainer(String containerId) throws CloudezzDeployException {
		this.stopContainer(containerId, 10);
	}

	public void stopContainer(String containerId, int timeout) throws CloudezzDeployException {
		restTemplate.postForLocation(dockerDeamonUrl + "/containers/{containerId}/stop?t={timeout}", null, containerId,
				timeout);
	}

	public void kill(String containerId) throws CloudezzDeployException {
		restTemplate.postForLocation(dockerDeamonUrl + "/containers/{containerId}/kill", null, containerId);
	}

	public void restart(String containerId, int timeout) throws CloudezzDeployException {
	...
	public String commit(CommitConfig commitConfig) throws CloudezzDeployException {
		...
		String response = restTemplate.postForObject(dockerDeamonUrl
				+ "/commit?container={container}&repo={repo}&tag={tag}&m={m}&author={author}&run={run}", null,
				String.class, params);
	...
	public InputStream build(File dockerFolder) throws CloudezzDeployException {
		return this.build(dockerFolder, null);
	}

	public InputStream build(File dockerFolder, String tag) throws CloudezzDeployException {
		...
		Preconditions.checkState(new File(dockerFolder, "Dockerfile").exists(), "Dockerfile doesn't exist in "
				+ dockerFolder);
		...
		try {
			...
			if (dockerFileContent.size() <= 0) {
				throw new CloudezzDeployException(String.format("Dockerfile %s is empty", dockerFile));
			}
			...
			for (String cmd : dockerFileContent) {
				if (StringUtils.startsWithIgnoreCase(cmd.trim(), "ADD")) {
					...
					if (addArgs.length != 3) {
						throw new CloudezzDeployException(String.format("Wrong format on line [%s]", cmd));
					}
					...
					if (!src.exists()) {
						throw new CloudezzDeployException(String.format("Source file %s doesnt' exist", src));
					}
					if (src.isDirectory()) {
			...
			throw new CloudezzDeployException("Error occurred while preparing Docker context folder.", ex);
		}
		...
		final ResponseEntity<String> response = restTemplate.exchange(dockerDeamonUrl + "/build?t={tag}",
				HttpMethod.POST, requestEntity, String.class, tag);

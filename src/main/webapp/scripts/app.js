...
var houstonApp = angular.module('houstonApp', [ 'http-auth-interceptor',
		'tmh.dynamicLocale', 'ngResource', 'ngRoute', 'ngCookies',
		...
		.run(
				[
						...
						function($rootScope, $location,
								AuthenticationSharedService, Account) {
											...
											function(event, next, current) {
												...
												if (current) {
													...
													if (template == "views/terminal.html") {
														$rootScope
																.$broadcast('event:close-term');
													}
												}
												// Check if the status of the
												// user. Is it authenticated or
												// not?
												$rootScope.page = $location
														.path();
												var currentPage = $location
														.path();
												if (currentPage
														.indexOf('/setpassword/') != 0) {
													AuthenticationSharedService
															.authenticate()
															.then(
																	function(
					...
					return function(input) {
						if (input != null && input != "") {
							...
							words = seconds < 45
									...
									|| hours < 24
									&& substitute(strings.hours, Math
											.round(hours), strings)
									|| hours < 42
									&& substitute(strings.day, 1, strings)
									|| days < 30
									&& substitute(strings.days, Math
											.round(days), strings)
									|| days < 45
